...
namespace TeenHackAPI.Data
{
    public class Get
    {
        public static List<Models.Exercises> GetExercisesByPurpose(Models.Purpose purpose,int id)
        {

            var cs = "Host=localhost;Username=postgres;Password=mityo1234;Database=teenhack";
            using var con = new NpgsqlConnection(cs);
            con.Open();
            List<Models.Exercises> exerciseList = new List<Models.Exercises>();
            for (int i = 1; i <= Data.NumOfExercises.ExercisesNum(); i++)
            {
                ...
                using (var command = new NpgsqlCommand("SELECT * FROM exercises WHERE id = " + i, con))
                {
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            if (exercise.purpose == purpose && id != exercise.id)
                            {
